...
class ROSTornadoBridgeSocketHandler(tornado.websocket.WebSocketHandler):
    sockets = set()
    ...
    def open(self):
        ...
        if not hasattr(self.ws_connection, "is_closing"):
            self.ws_connection.is_closing = types.MethodType(
                lambda self_: self_.stream.closed(
                ) or self_.client_terminated or self_.server_terminated,
                self.ws_connection
            )

        self.update_intervals_by_topic = {}  # this socket's throttle rate on each topic
        # last time this socket received data on each topic
        self.last_data_times_by_topic = {}

        ROSTornadoBridgeSocketHandler.sockets.add(self)

        self.write_message(json.dumps([ROSTornadoBridgeSocketHandler.MSG_SYSTEM, {
            "hostname": socket.gethostname(),
            "version": __version__,
        }], separators=(',', ':')))

    def on_close(self):
        ROSTornadoBridgeSocketHandler.sockets.remove(self)
    ...
    def send_pings(cls):
        ...
        for socket in cls.sockets:
            try:
                socket.last_ping_times[socket.ping_seq %
                                       1024] = time.time() * 1000
                if socket.ws_connection and not socket.ws_connection.is_closing():
                    socket.write_message(json.dumps([ROSTornadoBridgeSocketHandler.MSG_PING, {
                        ROSTornadoBridgeSocketHandler.PING_SEQ: socket.ping_seq,
                    }], separators=(',', ':')))
                socket.ping_seq += 1
    ...
    def broadcast(cls, message):
        ...
        try:
            if message[0] == ROSTornadoBridgeSocketHandler.MSG_TOPICS:
                json_msg = json.dumps(message, separators=(',', ':'))
                for socket in cls.sockets:
                    if socket.ws_connection and not socket.ws_connection.is_closing():
                        socket.write_message(json_msg)
            elif message[0] == ROSTornadoBridgeSocketHandler.MSG_MSG:
    ...
    def on_message(self, message):
        ...
        elif argv[0] == ROSTornadoBridgeSocketHandler.MSG_PONG:
            if len(argv) != 2 or type(argv[1]) is not dict:
                print("error: pong: bad: %s" % message)
                return

            received_pong_time = time.time() * 1000
            self.latency = (received_pong_time - self.last_ping_times[argv[1].get(
                ROSTornadoBridgeSocketHandler.PONG_SEQ, 0) % 1024]) / 2
            if self.latency > 1000.0:
                self.node.logwarn("socket %s has high latency of %.2f ms" % (
                    str(self.id), self.latency))

            if self.latency > 10000.0:
                self.node.logerr("socket %s has excessive latency of %.2f ms; closing connection" % (
                    str(self.id), self.latency))
                self.close()

        # client wants to subscribe to topic
        elif argv[0] == ROSTornadoBridgeSocketHandler.MSG_SUB:
            if len(argv) != 2 or type(argv[1]) is not dict:
        ...
        elif argv[0] == ROSTornadoBridgeSocketHandler.MSG_UNSUB:
            if len(argv) != 2 or type(argv[1]) is not dict:
...
ROSTornadoBridgeSocketHandler.MSG_PING = "p"
ROSTornadoBridgeSocketHandler.MSG_PONG = "q"
ROSTornadoBridgeSocketHandler.MSG_MSG = "m"
ROSTornadoBridgeSocketHandler.MSG_TOPICS = "t"
ROSTornadoBridgeSocketHandler.MSG_SUB = "s"
ROSTornadoBridgeSocketHandler.MSG_SYSTEM = "y"
ROSTornadoBridgeSocketHandler.MSG_UNSUB = "u"

ROSTornadoBridgeSocketHandler.PING_SEQ = "s"
ROSTornadoBridgeSocketHandler.PONG_SEQ = "s"
ROSTornadoBridgeSocketHandler.PONG_TIME = "t"
