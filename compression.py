...
def decode_jpeg(input_bytes):
    if simplejpeg:
        ...
        return cv2.imdecode(
            np.frombuffer(input_bytes, dtype=np.uint8), cv2.IMREAD_COLOR
        )[:, :, ::-1]
    elif PIL:
...
def decode_pcl2(cloud, field_names=None, skip_nans=False, uvs=[]):
    ...
    assert cloud.point_step * cloud.width * cloud.height == len(
        cloud.data
    ...
    for field in cloud.fields:
        ...
        assert (
            field.datatype in _PCL2_DATATYPES_NUMPY_MAP
    ...
    assert (
        cloud.point_step >= total_used_bytes
    ), "error: total byte sizes of fields exceeds point_step"

    if cloud.point_step > total_used_bytes:
        np_struct.append(
            ("unused_bytes", np.uint8, cloud.point_step - total_used_bytes)
        )
...
def compress_compressed_image(msg, output):
    ...
    if simplejpeg is None and cv2 is None and PIL is None:
        output[
            "_error"
        ] = "Please install simplejpeg, cv2 (OpenCV), or PIL (pillow) for image support."
        return
...
def compress_image(msg, output):
    ...
    if len(cv2_img.shape) == 3 and cv2_img.shape[2] == 2:
        cv2_img = np.stack(
            (cv2_img[:, :, 0], cv2_img[:, :, 1], np.zeros(cv2_img[:, :, 0].shape)),
            axis=-1,
        )
    ...
    if cv2_img.dtype != np.uint8:
        if cv2_img.dtype == np.uint64:
            ...
            cv2_img.dtype == np.float16
            or cv2_img.dtype == np.float32
            or cv2_img.dtype == np.float64
        ):
...
def compress_occupancy_grid(msg, output):
    ...
    try:
        occupancy_map = np.array(msg.data, dtype=np.uint16).reshape(
        ...
        # *10//4 is int approx to *255.0/100.0
        cv2_img = ((100 - occupancy_map) * 10 // 4).astype(np.uint8)
...
def compress_point_cloud2(msg, output):
    ...
    if "x" not in field_names or "y" not in field_names:
        output[
            "_error"
        ] = "PointCloud2 error: must contain at least 'x' and 'y' fields for visualization"
        return
    ...
    if np.little_endian:
        points_uint16 = (
            np.stack((xpoints_uint16, ypoints_uint16, zpoints_uint16), 1)
            .ravel()
            .view(dtype=np.uint8)
        )
    else:
...
def compress_laser_scan(msg, output):
    ...
    if len(ipoints) > 0 and len(ipoints) != len(rpoints):
        output[
            "_error"
        ] = "LaserScan error: intensities must be empty or equal in size to ranges"
        return
